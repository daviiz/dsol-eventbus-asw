...
public class Submarine extends Base/* extends  EventProducer  implements EventListenerInterface */ {
	...
	public Submarine(ModelData data, final DEVSSimulatorInterface.TimeDouble simulator)
		...
		//事件注册：
		this.addListener(Environment.getInstance(),Submarine.SUBMARINE_LOCATION_MSG);

		Environment.getInstance().addListener(_t1,Environment.ENVIRONMENT_SONAR_DETECTED);
		Environment.getInstance().addListener(_t2,Environment.ENVIRONMENT_SONAR_DETECTED);
